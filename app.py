...
st.set_page_config(
    page_title="Maluz Signal Engine V2.6",
    layout="wide"
)
...
st.title("🔹 Maluz Signal Engine V2.6")
st.caption(
...
# Keep the signal panel beside the upload area.
# It is populated later in the same Streamlit run once the
# detection, structure and confluence layers have finished.
upload_col, signal_col = st.columns([1, 1])
...
with signal_col:
    ...
    signal_slot.markdown(
        """
        <div style="border:1px solid #3b3f4a; border-radius:14px; padding:24px; min-height:180px; background:#11131a;">
            <div style="font-size:18px; font-weight:700; margin-bottom:18px;">🎯 SIGNAL</div>
            <div style="font-size:38px; font-weight:800; color:#9aa0aa;">WAITING</div>
            <div style="margin-top:12px; color:#9aa0aa;">Upload and analyse a chart to generate a signal.</div>
        </div>
        """,
...
if "candles" in st.session_state:
    ...
    # ========================================================
    # STEP 14 — SIGNAL ENGINE
    # ========================================================

    def generate_signal(setup_analysis, sequence):
        """
        STEP 14 — HARD-GATED BUY / SELL SIGNAL ENGINE

        A signal is generated only when the validated structural
        direction, current candle and confluence all agree.
        The confluence score is treated as a gate, NOT as a
        probability of winning.
        """

        structural_bias = str(
            setup_analysis.get("structural_bias", "UNKNOWN")
        ).upper()
        ...
        confluence_score = float(
            setup_analysis.get("confluence_score", 0.0)
        )

        current_confidence = float(
            sequence.get("current_confidence", 0.0)
        )
        ...
        event_index = last_event.get("candle_index")
        ...
        event_age = None
        if event_index is not None and current_index is not None:
            try:
                event_age = max(0, int(current_index) - int(event_index))
            except (TypeError, ValueError):
                event_age = None

        reasons = []
        gates = {
            "structural_direction": structural_bias in ("BULLISH", "BEARISH"),
            "structure_confirmed": "CONFIRMED" in str(
                setup_analysis.get("structure_status", "")
            ).upper(),
            "candle_aligned": candle_alignment == "ALIGNED",
            "candle_strength": candle_strength in ("STRONG", "MODERATE"),
            "event_aligned": event_alignment == "ALIGNED",
            "no_major_rejection": rejection_status == "NO MAJOR REJECTION",
            "confluence": confluence_score >= 75.0,
            "detection_confidence": current_confidence >= 80.0,
            "sequence_integrity": sequence_integrity >= 90.0,
        }

        if not gates["structural_direction"]:
            reasons.append("No confirmed bullish or bearish structural direction.")
        if not gates["structure_confirmed"]:
            reasons.append("Structure is not confirmed.")
        if not gates["candle_aligned"]:
            reasons.append("Current candle is not aligned with the structural direction.")
        if not gates["candle_strength"]:
            reasons.append("Current candle is too weak to trigger a signal.")
        if not gates["event_aligned"]:
            reasons.append("Latest BOS / CHoCH event does not confirm the setup direction.")
        if not gates["no_major_rejection"]:
            reasons.append("Major rejection is present.")
        if not gates["confluence"]:
            reasons.append("Confluence is below the signal threshold.")
        if not gates["detection_confidence"]:
            reasons.append("Current candle detection confidence is below the signal threshold.")
        if not gates["sequence_integrity"]:
            reasons.append("Candle sequence integrity is below the signal threshold.")

        if all(gates.values()) and setup_direction == "LONG":
            signal = "BUY"
            signal_icon = "🟢"
            signal_reason = "Bullish structure + aligned bullish candle + confirmed bullish event."
        elif all(gates.values()) and setup_direction == "SHORT":
            signal = "SELL"
            signal_icon = "🔴"
            signal_reason = "Bearish structure + aligned bearish candle + confirmed bearish event."
        else:
            signal = "NO SIGNAL"
            signal_icon = "⚪"
            signal_reason = reasons[0] if reasons else "Signal conditions are not fully satisfied."
    ...
    st.session_state["signal_analysis"] = signal_analysis

    # Populate the signal card created beside the upload area.
    signal = signal_analysis["signal"]
    signal_icon = signal_analysis["signal_icon"]
    ...
    age_text = (
        f"{signal_analysis['event_age']} candles"
        if signal_analysis["event_age"] is not None
        else "—"
    )

    signal_slot.markdown(
        f"""
        <div style="border:1px solid #3b3f4a; border-radius:14px; padding:24px; min-height:180px; background:{signal_bg};">
            <div style="font-size:18px; font-weight:700; margin-bottom:14px;">🎯 SIGNAL</div>
            <div style="font-size:46px; line-height:1.0; font-weight:900; color:{signal_color};">{signal_icon} {signal}</div>
            <div style="margin-top:16px; font-size:15px;">{signal_analysis['signal_reason']}</div>
            <div style="margin-top:14px; font-size:13px; color:#b9bdc8;">
                Confluence: {signal_analysis['confluence_score']:.1f}% &nbsp;•&nbsp;
                Detection: {signal_analysis['current_confidence']:.1f}% &nbsp;•&nbsp;
                Event age: {age_text}
            </div>
        </div>
        """,
    ...
    st.subheader("Signal Decision Audit")
    ...
    audit_col2.metric(
        "Signal Gate",
        "PASSED" if signal in ("BUY", "SELL") else "BLOCKED"
    )
    ...
    with st.expander("🔎 Signal decision audit", expanded=False):
        for gate_name, passed in signal_analysis["gates"].items():
            st.write(
                f"{'✅' if passed else '❌'} **{gate_name.replace('_', ' ').title()}**"
            )

        if signal_analysis["reasons"]:
            st.write("**Blocking / diagnostic reasons:**")
...
"""
    ...
    st.info(
        """
**Signal Engine:** BUY / SELL is generated only when the
structural, candle, event, detection-confidence and sequence
integrity gates all agree. The confluence score is a decision
gate, not a probability of winning.
"""
    )
